...
class Video(Process):
    def __init__(
        ...
        self.cage_zone1 = cage_zone1 if cage_zone1 is not None else [640, 0, 480, 0]
    ...
    def run(self):
        ...
        self.out_video = cv2.VideoWriter(
            self.path_video, self.fourcc_out, self.fps, (self.width, self.height)
        )
        ...
        while True:
            if self.daily_threshold:
                ...
                if begin_time < end_time:
                    ...
                    if day and now < time_utils.add_minutes_to_time(
                    ...
                    if not self.active:
                        ...
                        if self.cam_number == 2:
                            self.path_video = os.path.join(
                                self.path, self.name_video + "_tracking.avi"
                            )
                            self.path_npz = os.path.join(
                                self.path, self.name_video + "_tracking.npz"
                            )
                            self.path_csv = os.path.join(
                                self.path, self.name_video + "_tracking.csv"
                            )
                        self.path_video = os.path.join(
                            self.path, self.name_video + ".avi"
                        )
                        self.path_npz = os.path.join(
                            self.path, self.name_video + ".npz"
                        )
                        self.path_csv = os.path.join(
                            self.path, self.name_video + ".csv"
                        )
                        self.out_video = cv2.VideoWriter(
                            self.path_video,
                            self.fourcc_out,
            ...
            if self.cam_number == 1:
                ...
                if self.active:
                    (ret, self.frame), self.ctime = (
                        self.video.read(),
                        time_utils.now_datetime(),
                    )
                    if ret:
                        self.create_mask()
                        self.do_tracking_in()
    ...
    def do_tracking_out(self):
        [zcxmin1, zcxmax1, zcymin1, zcymax1] = self.cage_zone1
        ...
        cage_greyscale_frame1 = cv2.cvtColor(
            self.frame[zcymin1:zcymax1, zcxmin1:zcxmax1], cv2.COLOR_BGR2GRAY
        )
        cage_greyscale_frame2 = cv2.cvtColor(
            self.frame[zcymin2:zcymax2, zcxmin2:zcxmax2], cv2.COLOR_BGR2GRAY
        )
        doors1_greyscale_frame = cv2.cvtColor(
            self.frame[zdymin1:zdymax1, zdxmin1:zdxmax1], cv2.COLOR_BGR2GRAY
        )
        doors2_greyscale_frame = cv2.cvtColor(
            self.frame[zdymin2:zdymax2, zdxmin2:zdxmax2], cv2.COLOR_BGR2GRAY
        )

        cage_gaussian_frame1 = cv2.GaussianBlur(cage_greyscale_frame1, (5, 5), 0)
        ...
        cage_thresh1 = cv2.threshold(
            cage_gaussian_frame1, self.threshold2, 225, cv2.THRESH_BINARY_INV
        )[1]
        cage_thresh2 = cv2.threshold(
            cage_gaussian_frame2, self.threshold4, 225, cv2.THRESH_BINARY_INV
        )[1]
        doors1_thresh = cv2.threshold(
            doors1_gaussian_frame, self.threshold, 225, cv2.THRESH_BINARY_INV
        )[1]
        doors2_thresh = cv2.threshold(
            doors2_gaussian_frame, self.threshold3, 225, cv2.THRESH_BINARY_INV
        )[1]

        area_cage1 = cv2.countNonZero(cage_thresh1)
    ...
    def do_tracking_in(self):
        ...
        if self.tracking_inside:
            ...
            doors2_thresh = cv2.threshold(
                doors2_gaussian_frame, self.threshold, 225, cv2.THRESH_BINARY_INV
            )[1]
        ...
        if self.tracking_inside_floor:
            ...
            floor1_greyscale_frame = cv2.cvtColor(
                self.frame[zdymin3:zdymax3, zdxmin3:zdxmax3], cv2.COLOR_BGR2GRAY
            )
            floor2_greyscale_frame = cv2.cvtColor(
                self.frame[zdymin4:zdymax4, zdxmin4:zdxmax4], cv2.COLOR_BGR2GRAY
            )
            ...
            floor1_thresh = cv2.threshold(
                floor1_gaussian_frame, self.threshold, 225, cv2.THRESH_BINARY_INV
            )[1]
            floor2_thresh = cv2.threshold(
                floor2_gaussian_frame, self.threshold, 225, cv2.THRESH_BINARY_INV
            )[1]
    ...
    def do_tracking_position(self):
        if self.tracking_position:
            ...
            _, thresh = cv2.threshold(
                frame_mouse, self.threshold, 255, cv2.THRESH_BINARY_INV
    ...
    def get_output(self):
        ...
        self.out_video = cv2.VideoWriter(
            path, self.fourcc_out, self.fps, (self.width, self.height)
        )

    def add_info_trials(self):
        cv2.putText(
            self.frame,
            "trial: " + str(self.trial_number),
            (50, 100),
    ...
    def add_info_states(self):
        ...
        if (
            now_sec % 5 == 0 and now_sec != self.last_sec
        ):  # reset the fps counter each 5 sec
        ...
        cv2.putText(
            self.frame,
            "fps: " + str(self.fps_now) + " time: " + self.chrono.get_time_string(),
            (10, 60),
            cv2.FONT_HERSHEY_DUPLEX,
            0.5,
            (255, 255, 255),
    ...
    def add_info_tracking_out(self):
        try:
            ...
            cv2.putText(
                self.frame,
                f"Area in door1:    {self.area_doors1.value}",
            ...
            cv2.putText(
                self.frame,
                f"Area in door2:    {self.area_doors2.value}",
            ...
            cv2.putText(
                self.frame,
                f"Area total:        {self.area_total.value}",
                (settings.CAM1_TEXT_X, settings.CAM1_TEXT_Y + 80),
                cv2.FONT_HERSHEY_DUPLEX,
                0.4,
                (255, 0, 255),
                1,
            ...
            cv2.rectangle(
                self.frame, (zcxmin1, zcymin1), (zcxmax1, zcymax1), (0, 255, 0), 2
            )
            cv2.rectangle(
                self.frame, (zcxmin2, zcymin2), (zcxmax2, zcymax2), (0, 255, 0), 2
            )
            cv2.rectangle(
                self.frame, (zdxmin1, zdymin1), (zdxmax1, zdymax1), (0, 255, 255), 2
            )
            cv2.rectangle(
                self.frame, (zdxmin2, zdymin2), (zdxmax2, zdymax2), (255, 255, 0), 2
            )
    ...
    def add_info_tracking_in(self):
        if self.tracking_inside:
            try:
                ...
                cv2.putText(
                    self.frame,
                    f"Area in the box: {self.area_total.value}",
                    (10, 450),
                    cv2.FONT_HERSHEY_DUPLEX,
                    0.4,
                    (255, 0, 255),
                ...
                cv2.rectangle(
                    self.frame, (zdxmin1, zdymin1), (zdxmax1, zdymax1), (255, 0, 255), 2
                )
                cv2.rectangle(
                    self.frame, (zdxmin2, zdymin2), (zdxmax2, zdymax2), (255, 0, 255), 2
                )
                for key, value in self.centers.items():
                    cv2.circle(self.frame, value, 7, (255, 255, 255), -1)
        ...
        if self.tracking_inside_floor:
            try:
                ...
                cv2.putText(
                    self.frame,
                    f"Area in the floor: {self.area_total_floor.value}",
                    (10, 420),
                    cv2.FONT_HERSHEY_DUPLEX,
                    0.4,
                    (100, 0, 255),
                ...
                cv2.rectangle(
                    self.frame, (zdxmin3, zdymin3), (zdxmax3, zdymax3), (100, 0, 255), 2
                )
                cv2.rectangle(
                    self.frame, (zdxmin4, zdymin4), (zdxmax4, zdymax4), (100, 0, 255), 2
                )
    ...
    def add_info_tracking_position(self):
        if self.tracking_position:
            ...
            cv2.putText(
                self.frame,
                "threshold mouse: " + str(self.threshold),
                (10, 200),
                cv2.FONT_HERSHEY_DUPLEX,
                0.4,
                (0, 255, 0),
    ...
    def save_video1(self):
        self.out_video.release()


    def save_video2or3(self):
        self.out_video.release()
...
try:
    cam1 = Video(
        ...
        cage_zone1=settings.CAM1_CAGE_ZONE1,
...
try:
    cam2 = Video(
        port=settings.CAMERA2_PORT,
        cam_number=settings.CAM2_NUMBER,
        name_video=settings.CAM2_NAME_VIDEO,
        path=settings.VIDEOS_DIRECTORY,
