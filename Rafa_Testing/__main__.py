...
def main_loop():
    ...
    while True:
        ...
        if begin_time < end_time:
            ...
            if day and now < time_utils.add_minutes_to_time(begin_time, settings.DETECTION_WAITING_MINUTES):
                changing_light = True
            elif not day and now < time_utils.add_minutes_to_time(end_time, settings.DETECTION_WAITING_MINUTES):
                changing_light = True
        ...
        if utils.state == 0:  # waiting
            ...
            if status == 's':
                try:
                    ...
                    if not changing_light:
                        subject_action(first_time=True, last_tags=last_tags)
                except:
                    utils.log_cam('Academy', 'Subject not found for tag: ' + tag, 'ERROR')

            elif status is None:
                if time_utils.now_seconds() < last_time_tag + settings.DURATION_TAG and not changing_light:
                    subject_action(first_time=False, last_tags=last_tags, changing_light=changing_light)
