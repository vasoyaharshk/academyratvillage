...
class BpodIO(BpodCOMProtocolModules):
    ...
    def __init__(self, serial_port=None, workspace_path=None, session_name=None, sync_channel=None, sync_mode=None):
        self.workspace_path = workspace_path if workspace_path is not None else settings.SESSIONS_DIRECTORY
        self.session_name = session_name if session_name is not None else settings.SESSION_NAME
