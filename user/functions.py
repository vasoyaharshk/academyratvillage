...
#Stage dependent:
def forced_choice_only_correct_stage(path: str) -> str:
    # Apply forced-choice visual only from stage 2 onwards
    if (
        utils.task.stage > 1
        and utils.task.x_incorrecth is None
        and "both" in path
    ):
...
def function41():  # When the correct answer is on left
    ...
    left_images = []
    try:
        image_folder = f'/home/ratvillage01/academy/stimuli/webers_law/4_webers_law/{current_condition}'
