...
class Cognitive_Bias_Naming(Task):
    def __init__(self):
        super().__init__()
        ...
        self.image_display = 0  # Number of seconds the image will display after correct and incorrect. If there is no outvome display, set this to 0
    ...
    def main_loop(self):
        ...
        self.incorrect_freq = 1368.5

        # Set this at the start of the session:
        self.rat_name = "geralt"  # change this manually or via GUI
        self.reward_freq = self.rat_frequencies[self.rat_name]
        ...
        ### Assigining the correct tones to the correct stages, so that they must meet crit on stage 1, before going to stage 2:
        # Choose x positions:
        if self.stage == 1:
            self.stim = [211, 212]      #Stage 1 = tone pair 1 (2 khz is 211 vs 4 which is 212)
        if self.stage == 2:             #Tone pair 2, etc.
            self.stim = [213, 214]
        if self.stage == 3:
            self.stim = [215, 216]
        if self.stage == 4:
