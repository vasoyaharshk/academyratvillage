...
class StageTraining_7B_V1(Task):
    ...
    def main_loop(self):
        ...
        if self.current_trial == 0:
            ...
            print('Initial DL: '+str(self.init_stim_dur_dl))
        ...
        ####### STAGE 1: STIMULUS CATEGORIZATION ######
        if self.stage == 1:
            if self.substage == 1: # SUBSTAGE 1: STIMULUS REPOKING ALLOWED, LONG RESP WIN, MORE WATER
                self.valve_factor_c = 1.2
                self.valve_factor_i = 0.6

            self.duration_tired = 1800  # 30 mins
            # 5 initial easy trials: all VG
            if self.current_trial >= 5:
                self.pvg = 0.7
                self.pds = 0.3
        ...
        ####### Other stages #######
            ...
            # 3 initial easy trials: all VG, repoking allowed
            if self.current_trial > 3 and self.current_trial <= 7:  # 4 following intermediate trials
                self.pvg = 0.4
                self.pds = 0.6
                self.response_duration = 30
            ...
            ####### STAGE 2: DELAYS LEARNING #######
            elif self.stage == 2 and self.current_trial > 8:
                ...
                elif self.substage == 2:    # SUBSTAGE 2: DM CONSOLIDATION
                ...
                elif self.substage == 3:   # SUBSTAGE 3: DL CONSOLIDATION
                    self.response_duration = 15
                    self.stim_dur_ds = 0
                    self.stim_dur_dm = 0
                    self.pvg = 0.1
