...
class A_TouchTeaching_blue(Task):
    def __init__(self):
        super().__init__()
        ...
        Stages:
        Stage 1: A blue rectangle the same size and same location as the pegs. Rat has to touch the blue square but also can touch anywhere else.
        Stage 2: A blue rectangle the same size and same location as the pegs. Rat has to touch the blue square but if touches anywhere else is incorrect.
        Stage 3: Blue and yellow rectangle the same size and same location as the pegs. Rat has to touch the blue square but if touches anywhere else is incorrect.
    ...
    def main_loop(self):
        ...
        # Define correct and incorrect positions depending on stage
        if self.stim_trial == 213:
            # Correct = left position
            self.x_correcth = self.x_correcth_pos[0]
            # In Stage 3, incorrect = right (yellow); otherwise no explicit incorrect square
            self.x_incorrecth = self.x_correcth_pos[1] if self.stage == 3 else None
        elif self.stim_trial == 214:
            # Correct = right position
            self.x_correcth = self.x_correcth_pos[1]
            # In Stage 3, incorrect = left (yellow); otherwise no explicit incorrect square
            self.x_incorrecth = self.x_correcth_pos[0] if self.stage == 3 else None
