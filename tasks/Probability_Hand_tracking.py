...
class Probability_Hand_tracking(Task):
    def __init__(self):
        super().__init__()
        ...
        self.accwindow = [0]
        self.correct_count = 0          #Correct counter
        self.accuracy = 0
        self.trial_counter = 0          #Trial counter in total
        self.random_counter = 0         #Trial counter for randomisation
        self.random_block = 40           #Block of trials that the randomisation is for
    ...
    def main_loop(self):
        ...
        print('Random Counter: ' + str(self.random_block))
        print('Accuracy: ', self.accuracy)

        if self.random_block == 0:
        ...
        if self.random_block % 10 == 0 and self.bias_breaking == 0:  # Re-randomize every 10 trials
            # If not the first block, pass the last stimulus of the previous block to avoid repetition
            last_trial = self.stim_trials[self.random_block - 1] if self.random_block > 0 else None
            self.stim_trials = self.generate_random_trials(last_trial)
            # print('x positions list: ' + str(self.stim_trials))

        self.stim_trial = self.stim_trials[self.random_block]
