...
class Probability_Extra_Training_Bias_Left_Acc(Task):
    def __init__(self):
        ...
        self.stage = 1
        self.substage = 0
        self.substage_bias = 1  # 1 = 90:10, 2 = 75:25, 3 = 50:50
    ...
    def main_loop(self):
        ...
        self.bias_breaking = 0 #Bias breaking is always 0 in the side bias scripts
        ...
        if self.stage_forward_change == 1:
            ...
            if self.substage_bias == 3:
                self.task = 'Probability_Extra_Training_Acc'
                self.tired = True
        ...
        if self.stage_backward_change == 1:
            ...
            self.block_correct_count = 0
            self.block_valid_count = 0
            new_stage = max(self.substage_bias - 1, 1)
            if new_stage == self.last_forward_stage:
                ...
                self.moved_back_counter = 1
                self.last_backward_stage = new_stage
            self.substage_bias = new_stage
            message = f"Stage moved backward to {self.substage_bias} for {self.subject} in {self.task}"
            try:
                telegram_bot.alarm_finish_session(message, self.subject)
            except:
        ...
        ############ STATE MACHINE ################
        # First trial:
        if self.task == 'Probability_Extra_Training_Bias_Left_Acc':
            if self.current_trial == 0:
                self.sma.add_state(
