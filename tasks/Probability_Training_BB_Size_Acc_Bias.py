...
class Probability_Training_BB_Size_Acc(Task):
    def __init__(self):
        ...
        self.bias_break_size = 'small'
        self.bias_break_spacer = 'spacer'

        def bias_probability(bias_label):
            """
            Returns probability distribution for a given bias label based on the current substage_bias.

            - For substage_bias = 1: left, small and spacer presensce bias = [0.9, 0.1] and right, big and no spacer preset = [0.1, 0.9]
            - For substage_bias = 2:  same as in subsage_bias 1 but the probabilities are [0.75, 0.25] or [0.25, 0.75]
            - If none are met: neutral [0.5, 0.5]
            """
    ...
    def generate_random_trials_position_size_spacers(self, last_trial=None):
        ...
        size_map = {'small': [101, 102, 105, 106], 'big': [103, 104, 107, 108]}
        side_map = {'left': [101, 103, 105, 107], 'right': [102, 104, 106, 108]}
        spacer_map = {'spacer': [105, 106, 107, 108], 'no_spacer': [101, 102, 103, 104]}

        num_small = round(self.probabilities_size[0] * self.block_size)
        num_big = self.block_size - num_small
        num_left = round(self.probabilities_side[0] * self.block_size)
        num_right = self.block_size - num_left
        num_spacer = round(self.probabilities_spacer[0] * self.block_size)
        num_no_spacer = self.block_size - num_spacer

        trial_pool = []
        trial_pool += random.choices(size_map['small'], k=num_small)
        trial_pool += random.choices(size_map['big'], k=num_big)
        trial_pool += random.choices(side_map['left'], k=num_left)
        trial_pool += random.choices(side_map['right'], k=num_right)
        trial_pool += random.choices(spacer_map['spacer'], k=num_spacer)
        trial_pool += random.choices(spacer_map['no_spacer'], k=num_no_spacer)

        random.shuffle(trial_pool)
        trials = []

        for candidate in trial_pool:
            if len(trials) == 0 and last_trial is not None:
                if (
