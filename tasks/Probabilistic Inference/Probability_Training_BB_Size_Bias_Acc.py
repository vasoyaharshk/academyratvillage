...
class Probability_Training_BB_Size_Bias_Acc(Task):
    def __init__(self):
        super().__init__()

        self.info = """
        This script is IF THE BIAS IS TOWARDS THE LEFT SIDE.  So the stim will be displayed on the RIGHT more than the LEFT.
        ...
        Substage_bias regulates the probabilities on both sides:
        Substage_bias 1: 10% on left and 90% on right.
        Substage_bias 2: 25% on left and 75% on right.
        ...
        self.substage_bias = 1  # Side bias stage for substage behavior
        ...
        self.valve_factor_c = 1.4 # Normal water delivery must be a multiple of 25ul. 2.0 is 2 x 25 = 50uL. E.g., if you set it to 1.8, this would be 1.8 x 25 = 45uL
        ...
        #Bias Script Variables:
        self.substage_bias = 1
        self.bias_break_side = 'left'
        self.bias_break_size = None
        self.bias_break_spacer = None

        def bias_probability(bias_label):
            if self.substage_bias == 1:
                return [0.9, 0.1] if bias_label in ['left', 'small', 'spacer'] else [0.1, 0.9]
            if self.substage_bias == 2:
                return [0.75, 0.25] if bias_label in ['left', 'small', 'spacer'] else [0.25, 0.75]
            return [0.5, 0.5]

        self.probabilities_side = bias_probability(self.bias_break_side)
        self.probabilities_size = bias_probability(self.bias_break_size)
        self.probabilities_spacer = bias_probability(self.bias_break_spacer)


    def configure_gui(self):
        self.gui_input = ['stage', 'substage', 'duration_max']

    def generate_random_trials_position_size_bias(self, last_trial=None):
        print(f"Starting generate_random_trials_position_size_bias with last_trial: {last_trial}")
        ...
        size_n = {
            'small': round(self.probabilities_size[0] * n),
            'big': n - round(self.probabilities_size[0] * n)
        }
        side_n = {
            'left': round(self.probabilities_side[0] * n),
            'right': n - round(self.probabilities_side[0] * n)
        }
        ...
        counts = {'small': 0, 'big': 0, 'left': 0, 'right': 0}
        trials = []
        pool = [101, 102, 103, 104] * 10
        random.shuffle(pool)

        for code in pool:
            s, p = code_features[code]

            if counts[s] >= size_n[s]: continue
            if counts[p] >= side_n[p]: continue

            if self.probabilities_size == [0.5, 0.5] and len(trials) >= 2:
                if get_size(trials[-1]) == get_size(trials[-2]) == s:
                    continue
            if self.probabilities_side == [0.5, 0.5] and len(trials) >= 2:
                if get_position(trials[-1]) == get_position(trials[-2]) == p:
                    continue
        ...
        if last_trial is not None:
            while (get_size(trials[0]) == get_size(last_trial) or
                   get_position(trials[0]) == get_position(last_trial)):
                random.shuffle(trials)

        print("Generated trials:", trials)
        return trials

    def generate_random_trials_position_size_spacers_bias(self, last_trial=None):
        ...
        def get_spacer(trial):
            return 'spacer' if trial in [105, 106, 107, 108] else 'no_spacer'
        ...
        spacer_n = {
            'spacer': round(self.probabilities_spacer[0] * n),
            'no_spacer': n - round(self.probabilities_spacer[0] * n)
        }

        code_features = {
            101: ('small', 'left', 'no_spacer'),
            102: ('small', 'right', 'no_spacer'),
            103: ('big', 'left', 'no_spacer'),
            104: ('big', 'right', 'no_spacer'),
            105: ('small', 'left', 'spacer'),
            106: ('small', 'right', 'spacer'),
            107: ('big', 'left', 'spacer'),
            108: ('big', 'right', 'spacer')
        }

        counts = {'small': 0, 'big': 0, 'left': 0, 'right': 0, 'spacer': 0, 'no_spacer': 0}
        trials = []
        pool = [101, 102, 103, 104, 105, 106, 107, 108] * 10
        random.shuffle(pool)

        for code in pool:
            s, p, sp = code_features[code]

            if counts[s] >= size_n[s]: continue
            if counts[p] >= side_n[p]: continue
            if counts[sp] >= spacer_n[sp]: continue
            ...
            if self.probabilities_side == [0.5, 0.5] and len(trials) >= 2:
                if get_position(trials[-1]) == get_position(trials[-2]) == p:
                    continue
            if self.probabilities_spacer == [0.5, 0.5] and len(trials) >= 2:
                if get_spacer(trials[-1]) == get_spacer(trials[-2]) == sp:
                    continue
        ...
        if last_trial is not None:
            while (get_size(trials[0]) == get_size(last_trial) or
                   get_position(trials[0]) == get_position(last_trial) or
                   get_spacer(trials[0]) == get_spacer(last_trial)):
                random.shuffle(trials)
    ...
    def get_stim_image_path(self, stim_trial, stage):
        ...
        try:
            ...
            if stage == 1:
                image_folder = '/home/ratvillage03/academy/stimuli/urn_training/1_indication'
            elif stage == 2:
                image_folder = '/home/ratvillage03/academy/stimuli/urn_training/2_discrimination_a'
            elif stage == 3:
                image_folder = '/home/ratvillage03/academy/stimuli/urn_training/3_discrimination_b'
            elif stage == 4:
                image_folder = '/home/ratvillage03/academy/stimuli/urn_training/4_discrimination_c'
            else:
    ...
    def main_loop(self):
        ...
        if self.stage_forward_change == 1:
            ...
            self.last_forward_stage = self.substage_bias  # Save current BEFORE increasing
            self.substage_bias += 1
            ...
            if self.substage_bias == 3:
                self.task = 'Probability_Training_BB_Size_Acc'
                self.task_number = 2
                self.tired = True
        ...
        if self.task_number == 0:
            ...
            if self.stage == 4:
                if self.stim_trial_counter % self.block_size == 0 and self.bias_breaking == 0:  # Re-randomize every 20 trials
                    ...
                    self.stim_trials = self.generate_random_trials_position_size_spacers_bias(self.last_stim_trial)
                ...
                if self.stim_trial_counter % self.block_size == 0 and self.bias_breaking == 0:  # Re-randomize every 10 trials
                    # If not the first block_size, pass the last stimulus of the previous block_size to avoid repetition
                    self.last_stim_trial = self.stim_trials[self.stim_trial_counter - 1] if self.stim_trial_counter > 0 else None
                    self.stim_trials = self.generate_random_trials_position_size_bias(self.last_stim_trial)
                    print(f"Stimulus trials after first attempt: {self.stim_trials}")
                    while self.stim_trials is None:
                        print("Retrying to generate stimulus trials...")
                        self.stim_trials = self.generate_random_trials_position_size_bias(self.last_stim_trial)
        ...
        ############ STATE MACHINE ################
        #First trial:
        if self.task == 'Probability_Training_BB_Size_Left_Acc':
            if self.current_trial == 0:
                self.sma.add_state(
            ...
            print("Task Side bias ended. Task is now the normal task so will move to Core training in next session.")
    ...
    def after_trial(self):
        ...
        #Bias script:
        self.register_value('bias_break_side', self.bias_break_side)
        self.register_value('bias_break_size', self.bias_break_size)
        self.register_value('bias_break_spacer', self.bias_break_spacer)
        self.register_value('probabilities_side', self.probabilities_side)
        self.register_value('probabilities_size', self.probabilities_size)
        self.register_value('probabilities_spacer', self.probabilities_spacer)
